...
This script loads data from multiple trait and allometry data bases (including TRY) and organises into a tidy data set, with rows identifying observations, and columns identifying species, traits, and allometric data.

**Important**. This script works only on files that go through the pre-processing script, which will turn files into comma-separated values and remove all diacritical markings and non-basic characters, and split data into files according to the data provider.  This helps to load data more efficiently, but it can take a long time when running the script for the first time.
...
main_path       = file.path(home_path,"Data","TRY_DataBase")
...
* **TRYName**. Name of the ancillary data _exactly_ as in the TRY data base. 
...
```
Name,TryName,Desc,Type,UseStdUnit,Unit,Add,Mult,Power,Supporting
lon,Longitude,Longitude,numeric,TRUE,degE,0,1,1,NA
lat,Latitude,Latitude,numeric,TRUE,degN,0,1,1,NA
alt,Altitude,Altitude,numeric,TRUE,m,0,1,1,NA
ymean_prec,Mean sum of annual precipitation (PPT / MAP / TAP),Annual precipitation,numeric,FALSE,mm,0,1,1,NA
ymean_temp,Mean annual temperature (MAT),Mean annual temperature,numeric,FALSE,degC,0,1,1,NA
biome,Vegetation type / Biome,Biome,character,FALSE,empty,0,1,1,NA
biome_aux,Vegetation type / Biome ( 2),Biome,character,FALSE,empty,0,1,1,biome
sun_shade,Canopy position: sun vers. Shade leaf qualifier; light exposure,Sun/Shade Leaf,character,FALSE,empty,0,1,1,NA
lon_aux,Longitude (decimal degrees),Longitude,numeric,FALSE,degE,0,1,1,lon
lat_aux,Latitude (decimal degrees),Latitude,numeric,FALSE,degN,0,1,1,lat
climate_pft,Climate_PFT_Biome equivalent_Poulter,Climate PFT Biome,character,FALSE,empty,0,1,1,NA
```
...
height,3106,Plant height,numeric,0,cm.2.m,1,FALSE,m,FALSE,FALSE,FALSE,TRUE,FALSE,log,NA
height_3107,3107,Plant height,numeric,0,cm.2.m,1,FALSE,m,FALSE,FALSE,FALSE,TRUE,FALSE,log,height
...
wood_psixx,719,Xylem hydraulic vulnerability,numeric,0,1,1,FALSE,mpa,FALSE,FALSE,FALSE,FALSE,FALSE,neglog,NA
```
...
ancil_file = file.path(list_path,"TRY_AncillaryList.csv") # List of ancillary variable settings
trait_file = file.path(list_path,"TRY_TraitList.csv"    ) # List of TRY trait settings
```
...
extra_file_base = c("NGEE-Tropics.csv","LT-Brazil_Extra.csv","FrenchGuiana_Extra.csv"
                   ,"TalloNeoTropical.csv","SLBrazil_AllometryData.csv")
...
In addition, it is possible to define  a file name containing a comprehensive list of species belonging to the region of interest, that should always be included in model fitting. If none is available, it is fine to set `taxon_include=NA_character_`. The structure of this file should be similar to `taxon_file`, except that the only column used is the **AcceptedName** one.
...
# List of species to be always included in the 
taxon_include = file.path(main_path,"NeoTropical_Species.csv")
```
...
if (is_dry_run){
   ...
   try_trait   = read_csv(file=trait_file,col_types="ciccccclclllllc")   
...
The following chunk loads the list of genera to be always included in the analysis. If no list exists, the script will strictly follow the geographic information from the TRY data base.
```{r, label='load-genera'}
if ( is.na(taxon_include) ){
   cat0(" + List of required genera not provided. Use default settings.")
}else if ( file.exists(taxon_include) ){
   cat0(" + Load list of genera to be included whenever possible.")
   TaxonRef = read_csv(file=taxon_include,show_col_types=FALSE)
}else{
   stop(" + The reference list of species (",taxon_include,") was not found!")
}
...
if (reload_lookup && file.exists(Rdata_LookUp)){
   # Load data.
   ...
   AncilAll = tibble( Name       = character(0L)
   ...
   for ( f in seq_along(try_file_list)){
      ...
      AncilNow = TRYdbNow                                        %>%
         filter(is.na(TraitID))                                  %>%
         mutate( Name       = DataName
               , OrigValid  = is.valid(OrigValueStr,qq.rm=TRUE)
               , OrigUnits  = OrigUnitStr
               , StdValid   = is.valid(StdValue    ,qq.rm=TRUE)
               , StdUnits   = UnitName                          ) %>%
         select(c(Name,OrigValid,OrigUnits,StdValid,StdUnits))
   ...
   AncilLUT = AncilAll                                           %>%
      mutate( AncilID = match(Name,sort(unique(Name))))          %>%
      group_by(AncilID)                                          %>%
      summarise( CntUniqName  = length.unique(Name)
               , CntName      = length(Name)
               , CntOrigUnits = length.unique(OrigUnits)
      ...
      select(c(Name,CntUniqName,CntName,OrigValid,OrigUnits,CntOrigUnits,StdValid,StdUnits,CntStdUnits))
  ...
  AncilOrigUnits = AncilAll                                                                  %>%
      ...
      summarise( Name         = commonest(Name,na.rm=TRUE)
               ...
               , OrigUnits    = sort(unique(OrigUnits))                                    ) %>%
      ungroup()                                                                              %>%
      select(AncilID,Name,OrigUnits,CntOrigUnits)
   AncilStdUnits = AncilAll                                                                  %>%
      ...
      summarise( Name         = commonest(Name,na.rm=TRUE)
               ...
               , StdUnits     = sort(unique(StdUnits))                                     ) %>%
      ungroup()                                                                              %>%
      select(AncilID,Name,StdUnits,CntStdUnits)
...
if (reload_TidyTRY && file.exists(Rdata_RawTRY)){
   ...
   loop_list       = seq(from=1L,to=n_try_file_list,by=1)
  ...
  for ( f in loop_list){
      ...
      for (w in sequence(CntWhichTrait)){
         ...
         XylemNow      = as.integer(TraitIDNow) %in% c(719L)
...
if (load_ancil){
  ...
  for ( f in loop_list){
      ...
      # Find out which lines have traits and which lines have ancillary information
      IsAncil       = is.na(trydb_now$TraitID) & (trydb_now$DataName %in% try_ancil$TRYName)
      WhichAncil    = unique(trydb_now$DataName[IsAncil])
      CntWhichAncil = length(WhichAncil)

      # Append trait variables in case they are not in the data base.
      for (w in sequence(CntWhichAncil)){
         AncilNameNow = WhichAncil[w]
         z            = match(AncilNameNow,try_ancil$TRYName)


         # This should not happen...
         if (! is.finite(z)) stop(paste0(" Unrecognised Ancillary Data Set: ",AncilNameNow,"."))
         ...
         # Append values and units
         sel = is.na(trydb_now$TraitID) & (trydb_now$DataName %in% AncilNameNow)
         idx = match(trydb_now$ObservationID[sel],TidyTRY$ObservationID)
         ...
         if (UseStdUnitNow){
            ...
            # Apply some edits to ensure as much data as possible can be retrieved
            AncilFixed = TRY_FixAncil_OrigValue_Str( AncilName  = AncilNameNow
                                                   , Type       = TypeNow
                                                   , AncilOrig  = AncilOrig
                                                   , UnitOrig   = UnitOrig
                                                   , NameOrig   = NameOrig
         ...
         if (any(AncilMess)){
            ...
            cat0(" Variable name: " ,NameNow     ,".")
            cat0(" TRY Name: "      ,AncilNameNow,".")
            cat0(" Author Name: "   ,unique(AncilAuthor),".")
            cat0(" Unexpected exception occurred. Type \"print(Trouble)\" for error.")
   ...
   TidyTRY     = TidyTRY %>% select_at(vars(order_names))
   # Save look-up tables
...
if (file.exists(Rdata_ExtraTRY)){
   ...
   for (e in sequence(CntExtra)){
      ...
      # Extract column names. For now this is hard coded but xylem vulnerability curve
      # at different conductivity losses is provided as a single trait ID in TRY. Suggestions
      # on how to make this more generalisable are welcome.
      ColNames  = names(ExtraNow)
      SubNames  = gsub(pattern="xylem_p[0-9][0-9]",replacement="xylem_pxx",ColNames)
      ...
      for (w in sequence(CntAncilAppend)){
         ...
         DescNow       = try_ancil$Desc      [z]
         TypeNow       = try_ancil$Type      [z]
         # cat0("     ~ Add ancillary variable: ",DescNow," (",NameNow,").")
      ...
      ExtraNow  = ExtraNow %>% select_at(vars(KeepNames))
      ExtraNow  = ExtraNow %>%
...
if (file.exists(Rdata_GeoTaxonTRY)){
   ...
   IsGeoCoord = all(c("Longitude","Latitude") %in% try_ancil$TRYName)
   if (IsGeoCoord){
      cat0("   - Harmonise geographic coordinates")
      Lon            = try_ancil$Name[try_ancil$TRYName %in% "Longitude"]
      Lat            = try_ancil$Name[try_ancil$TRYName %in% "Latitude" ]
   ...
   }#end if (all(c("Longitude","Latitude") %in% try_ancil$TRYName))

   
   # Harmonise countries and continents.
   IsLocation = all(c("Location Country","Location Continent") %in% try_ancil$TRYName)
   if (IsGeoCoord & IsLocation){
      ...
      Lon       = try_ancil$Name[try_ancil$TRYName %in% "Longitude"]
      Lat       = try_ancil$Name[try_ancil$TRYName %in% "Latitude" ]
      Country   = try_ancil$Name[try_ancil$TRYName %in% "Location Country"        ]
      Continent = try_ancil$Name[try_ancil$TRYName %in% "Location Continent"      ]
   ...
   cat0("   - Harmonise taxonomy.")

   # Make sure taxize is set to silent.
   dummy = taxize_options(taxon_state_messages=TRUE,quiet=TRUE)
   ...
   if (IsGeoCoord & IsClimate){
      ...
      TRYPoints = TidyTRY %>% select_at(vars(c(Lon,Lat))) %>% filter(GeoIndex)
   ...
   if (use_realm %in% "NeoTropical"){
      ...
      Lon        = try_ancil$Name[try_ancil$TRYName %in% "Longitude"                           ]
      Lat        = try_ancil$Name[try_ancil$TRYName %in% "Latitude"                            ]
      Site       = try_ancil$Name[try_ancil$TRYName %in% "Location / Site Name"                ]
      Country    = try_ancil$Name[try_ancil$TRYName %in% "Location Country"                    ]
      Continent  = try_ancil$Name[try_ancil$TRYName %in% "Location Continent"                  ]
      Climate    = try_trait$Name[try_trait$TraitID %in% 825L                                  ]
      ClimatePFT = try_ancil$Name[try_ancil$TRYName %in% "Climate_PFT_Biome equivalent_Poulter"]
      Biome      = try_ancil$Name[try_ancil$TRYName %in% "Vegetation type / Biome"             ]
   ...
   if (length(UniqMiss) %gt% 0L) stop("Missing species in look-up table.")
   if (length(UniqMiss) %gt% 0L) cat0("     ~ Search remaining species online.")
   for (u in UniqMiss){
      UniqScientificNow = UniqScientific[u]
      cat ("       > ",sprintf("%4.4i of %4.4i",u,CntUniqTaxon),". Species taxonomy: ",UniqScientificNow,"...")
      UniqResolve                 = TNRS( UniqScientificNow,sources = "wfo",classification = "wfo")
      AcceptedName                = UniqResolve$Accepted_name
      if ( is.na(AcceptedName) | (AcceptedName %in% "") ){
         cat0(" Name not resolved.")
      }else{
         cat0(" Name resolved.")
         UniqSplit                   = strsplit(x=AcceptedName,split=" ")[[1]]
         UniqTaxon$ScientificName[u] = paste(UniqSplit[sequence(min(2L,length(UniqSplit)))],collapse=" ")
         UniqTaxon$Genus         [u] = UniqSplit[1]
         UniqTaxon$Family        [u] = UniqResolve$Accepted_family
      }#end if ( (! is.na(AcceptedName)) & (! AcceptedName %in% "") )
   }#end for (u in sequence(CntUniqTaxon))
   ...
   MissFamily    = sort(unique(UniqTaxon$Family[MissHighTaxa]))
   CntMissFamily = length(MissFamily)
   if (CntMissFamily %gt% 0L) cat0("     ~ Search higher taxonomy information online.")
   for (f in sequence(CntMissFamily)){
      UniqFamilyNow = MissFamily[f]
      cat ("       > ",sprintf("%4.4i of %4.4i",f,CntMissFamily),". Family taxonomy: ",UniqFamilyNow,"...")
      UniqHigher            = classification(UniqFamilyNow,db="ncbi")[[1]]
      UniqOrder             = UniqHigher$name[UniqHigher$rank %in% "order" ]
      UniqClass             = UniqHigher$name[UniqHigher$rank %in% "class" ]
      UniqPhylum            = UniqHigher$name[UniqHigher$rank %in% "phylum"]
      
      idx                   = which(UniqTaxon$Family %in% UniqFamilyNow)
      UniqTaxon$Order [idx] = UniqOrder
      UniqTaxon$Class [idx] = UniqClass
      UniqTaxon$Phylum[idx] = UniqPhylum
      cat0(" Done.")
   }#end for (f in sequence(CntUniqFamily))
   ...
   if (any(try_trait$TraitID %in% 42L)){
      ...
      Height      = try_trait$Name[try_trait$TraitID %in% 3106L]
...
if (file.exists(Rdata_TidyTRY)){
   ...
   # Replace generic vulnerability curve variable with the specific ones used in this site:
   ...
   if (use_realm %in% "NeoTropical"){
      # Build a subset of the tibble for testing tree likelihood.
      Biome     = try_ancil$Name[try_ancil$TRYName %in% "Vegetation type / Biome"  ]
      Habitat   = try_ancil$Name[try_ancil$TRYName %in% "Habitat / plot description"]
      EmptyChar = rep(NA_character_,times=nrow(TidyTRY))
   ...
   TidyTRY  = TidyTRY %>% select_at(vars(KeepTRY))
