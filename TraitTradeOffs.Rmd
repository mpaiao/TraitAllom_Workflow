...
if (plastic_sun_only){
   ...
   # Define photoplastic traits to filter for based on the chosen use_realm
    if (use_realm %in% c("NeoTropical", "PanTropical", "AsiaTropical", "AfricaTropical")) {
        ...
        PlasticTraits = c("SLA", "leaf_nitrogen", "leaf_n_area", "single_leaf_area", "huber", "m_amax", "a_amax", "m_gsw", 
                          "leaf_phosphorus", "leaf_p_area", "leaf_thick", "leaf_longevity", "leaf_dens") 
    } else {
        stop(paste("Unknown use_realm:", use_realm))
    }#end if(use_realm == "NeoTropical", "PanTropical", "AsiaTropical", "AfricaTropical")
...
if (update_impute){
   ...
   while (iterate){
      ...
      # Make sure variable chosen for the x axis for trait SMA analyses (xsma_TraitID) has data
      xsma_TraitID_count <- TallyTRY[TallyTRY$Name == xsma_name, "Cnt"]
      if (xsma_TraitID_count == 0) {
        cat0(" + The count of observations for trait ", xsma_name, " is 0.") 
        stop(paste("Reset xsma_TraitID to trait with >0 observations in TallyTRY"))
