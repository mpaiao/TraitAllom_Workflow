...
method_gap_maxSE = c("firstSEmax","Tibs2001SEmax","globalSEmax","firstmax", "globalmax")[3L]
...
CategInfo = tidyr::tribble( ~TraitID, ~Class, ~TRYClass                     ,   ~Colour, ~Symbol, ~XYUse
                          ...
                          ,         28L,  "ZOO", "Zoochory"                 , "#AED5E3",      9L, NA_character_
                          ,         28L,  "ZOO", "Ornithochory"             , "#AED5E3",      9L, NA_character_
                          ,         28L,  "ZOO", "Mammalochory"             , "#AED5E3",      9L, NA_character_
                          ,         28L,  "AUT", "Autochory"                , "#FCA2AE",     13L, NA_character_
                          ,         28L,  "HYD", "Hydrochory"               , "#008B96",     18L, NA_character_
                          ,         28L,  "ANE", "Anemochory"               , "#E72521",     16L, NA_character_
...
distr_summ      = file.path(summ_path,paste0("TRY_InfoDistr_"     ,use_suffix,".csv"))
cluster_medoid  = file.path(summ_path,paste0("TRY_ClusterMedoid_" ,use_suffix,".csv"))
cluster_medians = file.path(summ_path,paste0("TRY_ClusterMedians_",use_suffix,".csv"))
...
if (update_impute){
   ...
   WhichCandidate = with( try_trait, which( ( Impute | Cluster ) & ( ! Allom ) ) )
   TraitCandidate = unique(c(TaxonImpute,names(DataTRY)[names(DataTRY) %in% try_trait$Name[WhichCandidate]]))
...
if (update_cluster){
   ...
   ExcludeName    = names(ImputedTRY) %in% try_trait$Name[try_trait$TraitID %in% ExcludeTraitID]
   ...
   ClusterGap = clusGapFlex( x = ScaledTRY
                           ...
                           , K_max       = cluster_kmax
                           , d_power     = 2
                           , n_mcarlo    = n_mcarlo_cluster
                           , mc_verb     = round(0.1 * n_mcarlo_cluster)
                           , verbose     = TRUE
                           , do.swap     = FALSE
                           , pamonce     = 6L
                           )#end clusGapFlex
   ...
   k_opt_gap = 1L + maxSE(f=ClusterInfo$gap[-1L],SE.f=ClusterInfo$gapSE[-1L],SE.factor=1,method="globalSEmax")
...
if (update_tidy_cluster){
   ...
   SummCluster = TidyTRY %>%
      select(all_of(c("Cluster",LeafPhen,WoodDens,SLA))) %>%
      filter(! is.na(Cluster))                           %>%
      mutate(across(all_of(SLA), ~ -1*.x))               %>%
      group_by(Cluster) %>%
      summarise( across(where(is.character), ~ commonest(.x,na.rm=TRUE) )
               , across(where(is.double   ), ~ mean     (.x,na.rm=TRUE) ) ) %>%
      ungroup() %>%
      arrange(across(all_of(LeafPhen),desc),across(all_of(c(WoodDens,SLA)))) %>% 
      mutate(across(all_of(SLA), ~ -1*.x))
