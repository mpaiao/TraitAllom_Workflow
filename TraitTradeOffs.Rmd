...
authors: "Marcos Longo, Mara St. Amant"
date: "2022-04-25"
...
This flag defines whether to exclude observations from traits that show high light plasticity but were not collected from sunny leaves. If `plastic_sun_only=TRUE`, the code will exclude traits that are considered photo-plastic, unless there is high likelihood that the trait measurements used leaves in full sun (the exception being shrubs living in closed canopy habitats, which are assumed to be understorey specialists). If `plastic_sun_only=FALSE`, the code will keep all trait measurements.
...
xphoto_TraitID  = c(a_amax=53L,a_vcmax=186L,a_jmax=269L,m_amax=40L,m_vcmax=185L,m_jmax=270L)[1L]
...
cluster_method = c("sil","gap","fix")[3L]
...
In some cases, the user may need to overwrite the original settings (e.g., to update which traits to use for imputation, cluster analysis, and which ones to filter for plasticity). This is typically not recommended as the results may become less reproducible.  If the user wants to overwrite some settings, set variables `update_trait_file` and `update_ancil_file` with the full path to trait list and ancillary list (variables `trait_file` and `ancil_file` in `TidyTraitAllomDB.Rmd`. Otherwise, set `update_trait_file` and `update_ancil_file` with `NA_character_`).

**Important**. Loading these files will **NOT** update all variable settings, only the ones relevant for this script. Notably, this will not update units because this would require running `TidyTraitAllomDB.Rmd` again.

```{r, label='update-files',message=FALSE,results='hide',warning=FALSE}
update_ancil_file = c(NA_character_,file.path(list_path,"TRY_AncillaryList.csv"))[2L] # List of ancillary variable settings
update_trait_file = c(NA_character_,file.path(list_path,"TRY_TraitList.csv"    ))[2L] # List of TRY trait settings
```
...
If needed, update columns in `try_trait` and `try_ancil` with new information. We only update the columns relevant to this script.

```{r, label='update-trait-ancil',message=FALSE,results='hide',warning=FALSE}
# First, remove supporting traits because they are not used in this analysis. 
#They may have been introduced when TidyTraitAllomDB.Rmd is run only partially.

if (! is.na(update_trait_file)){
   update_trait_base = basename(update_trait_file)
   cat0(" + Update settings for \"try_trait\" using columns from file ",update_trait_base,".")

   # Read trait file.
   new_try_trait   = read_csv(file=update_trait_file,col_types="ciccccclclllllllcc")


   # Replace generic vulnerability curve variable with the specific ones used in this workflow:
   if ("xylem_pxx" %in% new_try_trait$Name){
      cat0("   - Split xylem potential at various declines in hydraulic conductivity.")
      new_try_xylem  = new_try_trait %>% filter(grepl(pattern="xylem_pxx",x=Name))
      new_xylem_vars = strsplit(x=new_try_xylem$Supporting,split=";")[[1]]
      new_xylem_pval = as.integer(gsub(pattern="^xylem_p",replacement="",x=new_xylem_vars))
      new_try_xylem  = 
         new_try_xylem[rep(1,times=length(new_xylem_pval)),,drop=FALSE] %>%
         mutate( Name = new_xylem_vars
               , Desc = mapply(FUN=gsub,replacement=new_xylem_pval,x=Desc,MoreArgs=list(pattern="xx"))
               , Supporting = NA_character_
               )#end mutate
      new_try_trait = bind_rows(new_try_trait,new_try_xylem) %>%
         arrange(Name) %>%
         filter(! (Name %in% "xylem_pxx"))
   }#end if ("xylem_pxx" %in% new_try_trait$Name)

   # Replace generic leaf texture variable with the specific ones used in this workflow:
   if ("leaf_texture" %in% new_try_trait$Name){
      new_try_ltext  = new_try_trait %>% filter(grepl(pattern="leaf_texture",x=Name))
      new_ltext_vars = strsplit(x=new_try_ltext$Supporting,split=";")[[1]]
      new_ltext_prss = ! grepl(pattern="_f_",x=new_ltext_vars)
      new_ltext_desc = rep(NA_character_,times=length(new_ltext_vars))
      new_ltext_desc[new_ltext_vars %in% "leaf_f_tear" ] = "Leaf tensile strength"
      new_ltext_desc[new_ltext_vars %in% "leaf_f_punct"] = "Leaf puncturability strength"
      new_ltext_desc[new_ltext_vars %in% "leaf_t_tear" ] = "Leaf tensile toughness"
      new_ltext_desc[new_ltext_vars %in% "leaf_t_punct"] = "Leaf puncturability toughness"
      new_try_ltext  =
         new_try_ltext[rep(1,times=length(new_ltext_vars)),,drop=FALSE] %>%
            mutate( Name       = new_ltext_vars
                  , Desc       = new_ltext_desc
                  , Unit       = ifelse( test = new_ltext_prss
      ...
      new_try_trait = bind_rows(new_try_trait,new_try_ltext) %>%
         arrange(Name) %>%
         filter(! (Name %in% "leaf_texture"))
   }#end if ("xylem_pxx" %in% names(try_trait))

   # Update the list of traits and ancillary data
   if ("Supporting" %in% names(new_try_trait)){
      new_try_trait = new_try_trait %>% filter(is.na(Supporting)) %>% select( ! Supporting)
   }#end if ("Supporting" %in% names(try_trait)){

   # Match the data.
   cat0("   - Map new data onto the existing database.")
   idx_trait  = match(try_trait$Name,new_try_trait$Name)

   # Update relevant columns.
   cat0("   - Update data for columns needed in this script.")
   try_trait$LightPlastic = new_try_trait$LightPlastic[idx_trait]
   try_trait$Impute       = new_try_trait$Impute      [idx_trait]
   try_trait$Cluster      = new_try_trait$Cluster     [idx_trait]
   try_trait$SMA          = new_try_trait$SMA         [idx_trait]
   try_trait$Allom        = new_try_trait$Allom       [idx_trait]
   try_trait$Photo        = new_try_trait$Photo       [idx_trait]
   try_trait$Trans        = new_try_trait$Trans       [idx_trait]
}#end if (! is.na(update_trait_file))


if (! is.na(update_ancil_file)){
   update_ancil_base = basename(update_ancil_file)
   cat0(" + Update settings for \"try_ancil\" using columns from file ",update_ancil_base,".")

   # Read ancillary file.
   new_try_ancil   = read_csv(file=update_ancil_file,col_types="cicclccccllc")

   # Update the list of ancillary data
   if ("Supporting" %in% names(new_try_ancil)){
      new_try_ancil = new_try_ancil %>% filter(is.na(Supporting)) %>% select( ! Supporting)
   }#end if ("Supporting" %in% names(new_try_ancil)){


   # Match the data.
   cat0("   - Map new data onto the existing database.")
   idx_ancil  = match(try_ancil$Name,new_try_ancil$Name)

   # Update relevant columns.
   cat0("   - Update data for columns needed in this script.")
   try_ancil$Impute       = new_try_ancil$Impute      [idx_ancil]
   try_ancil$Cluster      = new_try_ancil$Cluster     [idx_ancil]
}#end if (! is.na(update_ancil_file))
```
...
if (plastic_sun_only){
  ...
  PlasticTraits = names(TidyTRY)[names(TidyTRY) %in% try_trait$Name[try_trait$LightPlastic]]
    ...
    for (w in seq_along(PlasticTraits)){
       ...
       if (use_realm %in% c("NeoTropical", "PanTropical", "AsiaTropical", "AfricaTropical")) {
        Keep = IsSun | ( IsOpen & (! IsShade) ) | ( IsClosed & IsShrub )
       } else if (use_realm == "WestUS") {
        Keep = !IsShade             
       } else {
        stop(paste("Add method for removing observations from shade for use_realm = ", use_realm))
       }#end if(use_realm %in% c("NeoTropical", "PanTropical", "AsiaTropical", "AfricaTropical"))
...
}#end if (! use_woody_level %in% "all_plants")
```
...
if (update_impute){
   ...
   while (iterate){
      ...
      # Make sure variable chosen for the x axis for trait SMA analyses (xsma_TraitID) has enough data
      xsma_TraitID_count <- TallyTRY[TallyTRY$Name == xsma_name, "Cnt"]
      if (xsma_TraitID_count %le% n_fit_min) {
        cat0(" + The count of observations for trait ", xsma_name, " is 0.") 
        stop(paste("Reset xsma_TraitID to trait with > ", n_fit_min, " observations in TallyTRY"))
      }#end if (TallyTRY[TallyTRY$Name == xsma_TraitID, "Cnt"] == 0)
      
      
      # Make sure variable chosen for the x axis for trait SMA analyses of photosynthesis traits (xphoto_TraitID) has data
      xphoto_TraitID_count <- TallyTRY[TallyTRY$Name == xphoto_name, "Cnt"]
      if (xphoto_TraitID_count == 0) {
        cat0(" + The count of observations for trait ", xphoto_name, " is 0.") 
        stop(paste("Reset xphoto_TraitID to trait with >0 observations in TallyTRY"))
      }#end if (TallyTRY[TallyTRY$Name == xphoto_TraitID, "Cnt"] == 0)
