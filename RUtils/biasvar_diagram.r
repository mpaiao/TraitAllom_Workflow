#==========================================================================================
#==========================================================================================
#     Function biasvar_diagram.  This is based on Taylor diagrams, but instead of the
# model variance, the axes are observed variance and residual variance.#
#
# INPUT:
...
# biasvar        -- In case you want to add more data to an existing bias-variance plot,
#                   you must provide the object generated by a previous call, so it will
#                   adjust the scale accordingly.  Otherwise, leave this as NULL.
# normali(s/z)e  -- Should the axes be normalised?
# ...            -- Other par options that you may want to send to plot.window.
#------------------------------------------------------------------------------------------
biasvar_diagram  <<- function ( obs
                              , mod
                              ...
                              , main            = "Bias-variance diagram"
                              , bias.lim        = NULL
                              , sigma.lim       = NULL
                              , nobias.line     = TRUE
                              , nobias.options  = list( col = "mediumpurple1"
                                                      , lty = "dotdash"
                                                      , lwd = 2
                                                      )#end list
                              , sigma.grid      = TRUE
                              , sigma.options   = list( col = "grey40"
                                                      , lty = "dotted"
                                                      , lwd = 1.2
                                                      )#end list
                              , rmse.grid       = TRUE
                              , n.rmse          = NULL
                              , rmse.options    = list( col = "purple3"
                                                      , lty = "dashed"
                                                      , lwd = 1.2
                                                      )#end list
                              , cex.main        = 1.0
                              ...
                              , biasvar         = NULL
                              , normalize
                              , normalise       = ifelse( ! is.null(biasvar)
                                                        , biasvar$normalise
   ...
   #---~---
   if (add){
      if (! biasvar$normalise){
         sigma.obs = sd(x=obs,na.rm=TRUE)
         if (biasvar$sigma.obs != sigma.obs){
            cat(" - Original sigma(obs) : ",biasvar$sigma.obs,"\n")
            cat(" - Current sigma(obs)  : ",sigma.obs        ,"\n")
            cat(" - Original 'normalise': ",biasvar$normalise,"\n")
            stop("When adding points, the original plot must be normalised!")
         }#end if (biasvar$sigma.obs != sigma.obs)
         #---~---
      }#end if (! biasvar$normalise)
      #---~---


      #--- Check for mismatch between options.
      if (normalise != biasvar$normalise){
         warning(" Using 'normalise' option from original plot instead...")
         normalise = biasvar$normalise
      }#end if
      #---~---
   }else{
      #--- First plot, save the normalise option.
      biasvar$normalise = normalise
      #---~---
   }#end if (add)
   #---~---
   ...
   #---~---
   biasvar$mean.obs      = mean  (x = obs            , na.rm = TRUE)
   biasvar$mean.mod      = sapply(X = mod, FUN = mean, na.rm = TRUE)
   biasvar$mean.res      = sapply(X = res, FUN = mean, na.rm = TRUE)
   biasvar$sigma.obs     = sd    (x = obs            , na.rm = TRUE)
   biasvar$sigma.mod     = sapply(X = mod, FUN = sd  , na.rm = TRUE)
   biasvar$sigma.res     = sapply(X = res, FUN = sd  , na.rm = TRUE)
   biasvar$rmse          = sqrt( biasvar$mean.res^2
                               + (n.mod - 1) * biasvar$sigma.res^2 / n.mod
                               )#end sqrt
   biasvar$df.obs        = n.obs - 1
   biasvar$df.mod        = ifelse( is.null(n.parms), n.mod - 1,n.mod-n.parms)
   biasvar$r.squared     = 1. - ( biasvar$sigma.res / biasvar$sigma.obs ) ^ 2
   biasvar$adj.r.squared = ifelse( is.null(n.parms)
                                 , NA_real_
                                 , 1. -   biasvar$df.obs    / biasvar$df.mod
                                      * ( biasvar$sigma.res / biasvar$sigma.obs ) ^ 2
                                 )#end ifelse
   #---~---
   ...
   biasvar$scale = ifelse(normalise,biasvar$sigma.obs,1.0)
   sigma.obs     = biasvar$sigma.obs / biasvar$scale
   sigma.res     = biasvar$sigma.res / biasvar$scale
   mean.res      = biasvar$mean.res  / biasvar$scale
   #---~---
   ...
   if (! add){
      ...
      biasvar$plot = list( bias.lim  = bias.lim , bias.at  = bias.at
   ...
   # the user.
   #---~---
   if ( any(mean.res < biasvar$bias.lim[1] | mean.res > biasvar$bias.lim[2])
      | any(sigma.res < biasvar$sigma.lim[1] | sigma.res > biasvar$sigma.lim[2]) ){
