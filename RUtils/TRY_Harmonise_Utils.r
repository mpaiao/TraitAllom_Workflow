...
TRY_LonLatToGeoInfo <<- function(lon,lat,geo_adm1_path,simplified=TRUE){
  ...
  for (Large in LargeCountries){
    ...
    #browser()
    #LargeSP = project(LargeSP,crs(CountriesSF))
    #LargeSF = st_as_sf(LargeSP)
...
SelectAfricaTropical <<- function(x){
  ...
  # in the tropics.
  #---~---
  AfricanCountries = c( "Angola","Botswana","Burundi", "Cameroon", "Cape Verde", 
                        "Central African Republic", "Chad", "Congo", "Benin", "Equatorial Guinea", "Ethiopia", 
                        "Djibouti", "Gabon", "Gambia", "Ghana", "Guinea", "Ivory Coast", "Kenya", "Liberia", "Madagascar", 
                        "Malawi", "Mali", "Mauritania", "Mozambique", "Namibia", "Niger", "Nigeria", "Guinea-Bissau", "Zimbabwe", 
                        "Rwanda", "Senegal", "Sierra Leone", "Somalia", "Sudan", "Tanzania", "Togo", "Uganda", "Burkina Faso", "Zaire", "Zambia"
  )#end c
